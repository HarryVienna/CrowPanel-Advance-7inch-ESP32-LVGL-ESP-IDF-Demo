...
static stc8h1k28_handle_t extender_handle = NULL; 
...
void init_extender(i2c_master_bus_handle_t i2c_bus_handle, stc8h1k28_handle_t *extender_handle) { 
    // Initialisierung des stc8h1k28 Treibers
    i2c_device_config_t dev_config = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = STC8H1K28_I2C_ADDR,
        .scl_speed_hz = I2C_CLK_SPEED_HZ, 
        .flags.disable_ack_check = true, // Disable ACK check
    };

    esp_err_t ret = stc8h1k28_init(i2c_bus_handle, extender_handle, &dev_config); 
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Fehler beim Initialisieren des stc8h1k28");
        //i2c_master_bus_delete(i2c_bus); // Bei Fehler I2C Bus wieder freigeben
        return;
    }
    
    ESP_LOGI(TAG, "stc8h1k28 erfolgreich initialisiert");
}
 ...
 */
void init_touch(i2c_master_bus_handle_t i2c_bus_handle, esp_lcd_touch_handle_t *touch_handle) {
 ...
 *
 * @param[in] brightness The brightness level to set (0-245).
 */
void set_backlight_brightness(uint8_t brightness) {
    ...
    // Brightness adjustment spans 0-245 levels, where 0 denotes maximum brightness, 
    // 244 represents minimum brightness, and 245 disables the backlight.
    brightness = 245 - brightness;

    stc8h1k28_set_brightness(extender_handle, brightness);
...
void beep(uint8_t duration) {
    ...
    stc8h1k28_buzzer_off(extender_handle);
