...
esp_err_t esp_lcd_touch_new_i2c_gt911(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch, pca9557_handle_t expander_handle)
{
    ...
    /* Uncomment if touch max coordinates do not fit display coordinates */
    // vTaskDelay(pdMS_TO_TICKS(50));

    // ret = touch_gt911_write_resolution(esp_lcd_touch_gt911, config->x_max, config->y_max);
    // ESP_GOTO_ON_ERROR(ret, err, TAG, "GT911 write resolution failed");
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_LOGI(TAG, "Start touch_gt911_read_cfg");
    ...
    ESP_RETURN_ON_ERROR(touch_gt911_i2c_read(tp, ESP_LCD_TOUCH_GT911_MAX_X, (uint8_t *)&x_res[0], 2), TAG, "GT911 read error!");
    ESP_RETURN_ON_ERROR(touch_gt911_i2c_read(tp, ESP_LCD_TOUCH_GT911_MAX_Y, (uint8_t *)&y_res[0], 2), TAG, "GT911 read error!");

    uint16_t x_value = (x_res[1] << 8) | x_res[0];
    uint16_t y_value = (y_res[1] << 8) | y_res[0];
    ESP_LOGI(TAG, "TouchPad X 1:%u", x_res[1]);
    ESP_LOGI(TAG, "TouchPad X 0:%u", x_res[0]);

    ESP_LOGI(TAG, "TouchPad X resolution:%u", x_value);
    ESP_LOGI(TAG, "TouchPad Y resolution:%u", y_value);
